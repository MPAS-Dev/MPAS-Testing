...
* namelist.input is used for actually running dome test case in the MPAS land ice core.  It includes most but not all options available to the model.  See the default namelist.input.landice file in the MPAS root directory for a list of all options available.  They are also documented in the User's Guide.
